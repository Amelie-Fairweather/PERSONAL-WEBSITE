...
const personalWebsites = [
  {
    name: "Eloise Glasscoe",
    url: "https://www.eloiseglasscoe.org",
    description: "Personal website showcasing portfolio and work",
    image: "/web.png"
  }
];
...
export default function PastworkPage() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (entry.target === personalSectionRef.current && entry.isIntersecting) {
    ...
    setTimeout(() => {
      ...
      if (personalSectionRef.current) {
        observer.observe(personalSectionRef.current);
        const rect = personalSectionRef.current.getBoundingClientRect();
        if (rect.top < window.innerHeight && rect.bottom > 0) {
          setIsPersonalVisible(true);
        }
      }
    ...
    return () => {
      ...
      if (personalSectionRef.current) {
        observer.unobserve(personalSectionRef.current);
      }
    };
  }, []);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[var(--offWhite)] font-sans">
      ...
      <div ref={personalSectionRef} className="w-full max-w-6xl mx-auto px-4 md:px-6 py-10 md:py-20" style={{
        transform: `translateY(${isPersonalVisible ? Math.max(0, Math.min(30, (offsetY - 300) * 0.05)) : 60}px)`,
        ...
        <div className="space-y-6 md:space-y-8">
          {personalWebsites.map((project, index) => (
            <div 
              key={index} 
              className="rounded-lg shadow-lg p-6 md:p-8 hover:scale-105 transition-transform duration-300 bg-[var(--offWhite2)]"
            >
              <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6">
                <div className="flex-1 w-full">
                  <div className="flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-4 mb-3">
